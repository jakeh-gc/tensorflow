  ...
  switch (shape.element_type()) {
    ...
    case F8E4M3FN:
      return SetValueInLiteralHelper<tsl::float8_e4m3fn>(loc, value, index,
                                                         literal);
    case F8E5M2FNUZ:
      return SetValueInLiteralHelper<tsl::float8_e5m2fnuz>(loc, value, index,
                                                       literal);
    case F8E4M3FNUZ:
      return SetValueInLiteralHelper<tsl::float8_e4m3fnuz>(loc, value, index,
                                                         literal);
    case F16:
...
bool HloParserImpl::SetValueInLiteralHelper(LocTy loc, ParsedElemT value,
  ...
  const auto handle_nan =
    ...
    if constexpr (std::is_same<LiteralNativeComponentT,
                               tsl::float8_e4m3fn>::value || std::is_same<LiteralNativeComponentT,
                               tsl::float8_e5m2fnuz>::value || std::is_same<LiteralNativeComponentT,
                               tsl::float8_e4m3fnuz>::value) {
          if (nan_payload != QuietNanWithoutPayload<double>()) {
            return Error(
                loc, StrCat("tries to set NaN payload 0x",
                            ...
                            ShapeUtil::HumanString(literal->shape()),
                            " at linear index ", index,
                            ", but f8e4m3fn, float8_e5m2fnuz, and float8_e4m3fnuz does not support payloads"));
          }
        } else {
          if (nan_payload == QuietNanWithoutPayload<double>()) {
